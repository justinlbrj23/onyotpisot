...
async function getUrlMapping() {
  try {
    const res = await sheets.spreadsheets.values.get({ spreadsheetId: SPREADSHEET_ID, range: `${SHEET_NAME_URLS}!A2:C` });
    const rows = res.data.values || [];
    const mapping = {};
    rows.forEach(([county, state, url]) => { if (url) mapping[url.trim()] = { county: county || '', state: state || '' }; });
    return mapping;
  } catch (err) { console.warn('Could not fetch URL mapping from sheet:', err.message || err); return {}; }
}
...
async function fetchExistingKeys() {
  try {
    const res = await sheets.spreadsheets.values.get({ spreadsheetId: SPREADSHEET_ID, range: `${SHEET_NAME_RAW}!A2:Z` });
    const rows = res.data.values || [];
    const keys = new Set();
    for (const r of rows) {
      const parcel = r[5] || '';
...
function mapRow(raw, urlMapping, anomalies) {
  if (!raw) return null;
  const mapped = {}; HEADERS.forEach(h => (mapped[h] = ''));
  const baseUrl = (raw.sourceUrl || '').split('&page=')[0];
  const geo = urlMapping[baseUrl] || { county: '', state: '' };
  mapped['State'] = geo.state; mapped['County'] = geo.county;
  ...
  mapped['Sale Price'] = salePrice; mapped['Opening / Minimum Bid'] = openingBid;
  const sale = parseCurrency(salePrice); const open = parseCurrency(openingBid);
  const estimatedSurplus = (sale !== null && open !== null) ? (open - sale) : (sale !== null ? null : null);
  if (sale === null && mapped['Sale Finalized (Yes/No)'] === 'Yes') anomalies.push({ type: 'MissingSalePrice', parcelId: raw.parcelId, sourceUrl: raw.sourceUrl });
...
async function appendRowsBatched(rows) {
  ...
  for (let i = 0; i < batches.length; i++) {
    ...
    await withRetry(() => sheets.spreadsheets.values.append({
      spreadsheetId: SPREADSHEET_ID, range: SHEET_NAME_RAW, valueInputOption: 'RAW', insertDataOption: 'INSERT_ROWS', requestBody: { values },
    }));
...
(async () => {
  try {
    if (!fs.existsSync(INPUT_FILE)) { console.error('Input file not found:', INPUT_FILE); process.exit(1); }
    ...
    const anomalies = []; const uniqueMap = new Map();
    ...
    if (anomalies.length) { fs.writeFileSync(ANOMALY_FILE, JSON.stringify(anomalies, null, 2)); console.log(`⚠️ Saved ${anomalies.length} anomalies → ${ANOMALY_FILE}`); }
    if (mappedRows.length && ENABLE_SHEET_IDEMPOTENCY) { await appendRowsBatched(mappedRows); console.log('✅ Appended mapped rows to sheet'); }
    else if (mappedRows.length) { console.log('ℹ️ Mapped rows present but idempotency disabled; not appending to sheet automatically.'); }
    else { console.log('ℹ️ No mapped rows to append after dedupe/idempotency checks.'); }
    console.log('🏁 Done.');
  } catch (err) { console.error('Fatal error in mappingScraper:', err && err.message ? err.message : err); process.exit(1); }
})();