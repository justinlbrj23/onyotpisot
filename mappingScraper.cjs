...
function extractPlainUrl(value) {
  const text = String(value || '').trim();
  if (!text) return '';

  const hrefMatch = text.match(/href\s*=\s*["']([^"']+)["']/i);
  if (hrefMatch) return decodeAmp(hrefMatch[1]).trim();

  const urlMatch = text.match(/https?:\/\/[^\s<>"']+/i);
  if (urlMatch) return decodeAmp(urlMatch[0]).trim();

  return decodeAmp(text).trim();
}

function normalizeBaseUrl(value) {
  const raw = extractPlainUrl(value);
  if (!raw) return '';
...
function getOrigin(value) {
  try { return new URL(extractPlainUrl(value)).origin.toLowerCase(); }
  catch { return ''; }
}
...
function isFinalizedRow(raw) {
  ...
  if (
    ...
    combined.includes('auction starts')
  ) {
    return false;
  }
  ...
  if (
    ...
    combined.includes('paid prior') ||
    combined.includes('paid in full') ||
  ...
  return parseCurrency(raw.salePrice) !== null;
}

function resolveGeo(raw, urlMapping) {
  const sourceUrl = extractPlainUrl(raw.sourceUrl || '');
  ...
  if ((!geo.county || !geo.state) && sourceUrl) {
    try {
      const inferred = inferCountyStateFromHost(new URL(sourceUrl).hostname);
      geo = {
...
function mapRow(raw, urlMapping, anomalies) {
  ...
  const estimatedSurplus = parseCurrency(raw.surplus);

  // Missing data is recorded for diagnostics only. It never removes a row.
  if (salePrice === null) {
    anomalies.push({
      type: 'MissingSalePrice',
      message: 'Sale price is currently unavailable. Row was still mapped.',
  ...
  if (estimatedSurplus === null) {
    anomalies.push({
      type: 'MissingSurplus',
      message: 'Canonical raw.surplus is currently unavailable. Row was still mapped.',
      parcelId: raw.parcelId || '',
      caseNumber: raw.caseNumber || '',
      sourceUrl: extractPlainUrl(raw.sourceUrl || ''),
    });
  ...
  mapped['Sale Finalized (Yes/No)'] = finalized ? 'Yes' : 'No';
  mapped['Sale Price'] = salePrice === null ? '' : String(salePrice);
  ...
  if (estimatedSurplus === null) {
    mapped['Meets Minimum Surplus? (Yes/No)'] = '';
    mapped['Deal Viable? (Yes/No)'] = '';
  } else {
    const meets = estimatedSurplus >= MIN_SURPLUS;
  ...
  if (!finalized) {
    mapped['Kickback Reason'] = 'Auction not finalized; awaiting final sale result';
  } else if (salePrice === null && estimatedSurplus === null) {
    mapped['Kickback Reason'] = 'Sale price and surplus unavailable';
  } else if (salePrice === null) {
    mapped['Kickback Reason'] = 'Sale price unavailable';
  } else if (estimatedSurplus === null) {
    mapped['Kickback Reason'] = 'Canonical surplus unavailable';
  }
  ...
  try {
    ...
    for (const original of rawData) {
      ...
      const key = [
        ...
        clean(raw.auctionId),
        clean(raw.auctionDate),
        clean(raw.propertyAddress).toUpperCase(),
      ].join('|');
